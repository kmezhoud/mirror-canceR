.test <- function(dir, pattern = ".*\\.R$")
{
    ...
    if (missing(dir)) {
        dir <- system.file("unitTests", package="canceR")
        if (!length(dir)) {
            dir <- system.file("UnitTests", package="canecR")
            if (!length(dir))
    ...
    suite <- defineTestSuite(name="canceR RUnit Tests", dirs=dir,
    ...
    if (length(details <- .failure_details(result)) >0) {
        ...
        cat("\n\n")
        stop("unit tests failed for package canceR ")
    }
    result
}
